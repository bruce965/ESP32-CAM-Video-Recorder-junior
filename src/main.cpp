...
void read_config_file() {
  ...
  if (config_file) {
    ...
    // lets make a config.txt config file
  ...
  Serial.printf("pass %s\n", cpass.c_str()); logfile.printf("pass %s\n", cpass.c_str());
...
void setup() {
  ...
  char filebuff[50];
  sprintf(filebuff, "/%s_filegroup.dat", devname);
  Serial.println("Reading last file group...");
  File file_group_file = SD_MMC.open(filebuff, FILE_READ);
  file_group = file_group_file.parseInt();  // returns 0 if empty
  file_group_file.close();

  Serial.printf("Incrementing last file group (previously %d)...", file_group);
  Serial.println();
  file_group_file = SD_MMC.open(filebuff, FILE_WRITE);
  int len = sprintf(filebuff, "%d", file_group + 1);
  file_group_file.write((uint8_t*)filebuff, len * (sizeof(filebuff[0]) / sizeof(uint8_t)));
  file_group_file.flush();
  file_group_file.close();

  sprintf(filebuff, "/%s%d.999.txt",  devname, file_group);
  Serial.printf("Creating logfile %s\n",  filebuff);
  logfile = SD_MMC.open(filebuff, FILE_WRITE);
